...
function run_samtools_faidx {
    ref=$1
    fai=${ref}.fai
    if [ ! "$(samtools faidx --version &>/dev/null)" ]; then
...
SRRLIST=$(grep -v "Run" SraRunInfo.csv | awk 'BEGIN {FS=","} {print $1}' | tr "\n" " " | sed -e "s/ $//")
SAMPLELIST=$(grep -v "Run" SraRunInfo.csv | awk 'BEGIN {FS=","}  {print $30}' | tr "\n" " " | sed -e "s/ $//")

read -a -r srr <<<"$SRRLIST"
read -a -r sample <<<"$SAMPLELIST"

if [ ! -e "sampleinfo.csv" ]; then
    cut SraRunInfo.csv -d "," -f 1,29,30 \
...
function run_variantfiltration {
    ...
    if [ ! -e "${outvcf}" ]; then
        gatk VariantFiltration -OVI true --variant "$invcf" --output "$outvcf" "$opts"
    fi
...
function run_genotype_gvcfs {
    ...
    if [ ! -e "${outvcf}" ]; then
        gatk GenotypeGVCFs -OVI true -R "$ref" -V "$combinevcf" -O "$outvcf" "$opts"
    fi
}
