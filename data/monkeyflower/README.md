...
The data can be generated by using the Snakefile located in the
workflow directory. The workflow rulegraph is depicted below:

![Snakemake workflow rulegraph](./rulegraph.svg)

## Dataset versions

[tiny](./tiny/README.md): tiny dataset for generating website

[small](./small/README.md): small dataset covering region LG4:11-14
Mbp.

Larger datasets can be prepared but are not hosted here. See the
[configuration file](./config/config.all.yaml) for an example.

## OBSOLETE: bash workflow
