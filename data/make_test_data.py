...
import numpy as np
import tskit
from utils.dataclasses import SeqRecord

logger = logging.getLogger(__name__)

logging.basicConfig(
    level=logging.INFO, format="%(levelname)s (make_test_data): %(message)s"
)
...
def ucsc_twobittofa(chromosome, start, end):
    url = "http://hgdownload.soe.ucsc.edu/goldenPath/hg38/bigZips/hg38.2bit"
    outfile = f"ucsc/{chromosome}-{start}-{end}.fa"
    if not os.path.exists("ucsc"):
        os.makedirs("ucsc")
    subprocess.run(["twoBitToFa", f"{url}:{chromosome}:{start}-{end}", outfile])  # noqa
...
class Individual:
    ...
    def __repr__(self):
        return (
            f"Individual(uniqueid={self.uniqueid}, id={self.id}, "
...
class DemesModel:
    ...
    def add_individuals(self, **kw):
        for pop, count in kw.items():
            ...
            for i in np.arange(popsize, popsize + count):
                ind = Individual(
                    uniqueid=self.census, id=i, population=popindex, population_name=pop
                )
    ...
    def make_reference_sequence(self):
        ...
        h0 = (
            f"{self.reference.tskit_id}-{self.reference.population_name}"
            f"-{self.reference.id}-0"
        )
        h1 = (
            f"{self.reference.tskit_id}-{self.reference.population_name}"
            f"-{self.reference.id}-1"
        )
        self.reference.haplotype = [
            SeqRecord(
                id=h0,
                name=h0,
                seq="".join(hap0),
                haplotype=0,
                population=self.reference.population_name,
            ),
            SeqRecord(
                id=h1,
                name=h1,
    ...
    def simulate(
        self, *, seqlength, recombination_rate, mutation_rate, anc_seed=42, mut_seed=10
    ):
...
def make_ooa(args):
    ...
    np.random.seed(52)
    model = DemesModel(
        name="ooa", path=pathlib.Path("ooa"), demesfile="ooa/ooa.demes.yaml"
    )
...
def download_ucsc(args):
    ucsc_twobittofa(args.chromosome, args.start, args.end)
...
def main():
    ...
    ooa_outgroups_parser = subparsers.add_parser(
        "ooa-outgroup-with-outgroups", help="Make ooa dataset (neutral) with outgroups"
    )
    ...
    ucsc_parser = subparsers.add_parser("ucsc", help="Download ucsc data")
    ucsc_parser.set_defaults(runner=download_ucsc)
    ucsc_parser.add_argument("--chromosome", help="chromosome name", default="chr22")
    ucsc_parser.add_argument(
        "--start", help="chromosome start position", default=11123467
    )
    ucsc_parser.add_argument("--end", help="chromosome end position", default=11173466)
